...
function list_employees_pay_table($table) {
    ...
    if ($table == 'begin') {
        ...
        $temp_pay_type_array = array(
            "Salary",
            "Hourly",
            "Overtime"
        ); //TODO grab this from the db - these should only be pay types that are currently assigned to employees
        $th                  = array(
            ("check"),
            _("Name")
        );
...
function create_payroll_cart() {
    ...
    while ($myrow = db_fetch($result)) {
        $cart->add_employee($myrow['emp_id'], $myrow['emp_first_name'] . " " . $myrow['emp_last_name'], $myrow['emp_payfrequency'], $myrow['emp_filingstatus'], $myrow['emp_allowances'], get_employee_pay_array($myrow['emp_id']), get_employee_ytd($myrow['emp_id']));
    }

    $result = get_all_payroll_tax_types();
    while ($myrow = db_fetch($result)) {
        $cart->add_tax($myrow['id'], $myrow['name'], $myrow['type'], $myrow['responsibility'], $myrow['allowance'], $myrow['payroll_gl_code'], get_payroll_tax_rates($myrow['id']));
    }
